...
-- Supports different permission scopes and rate limiting.
...
COMMENT ON COLUMN api_keys.rate_limit_per_minute IS 'Max requests per minute';
COMMENT ON COLUMN api_keys.rate_limit_per_day IS 'Max requests per day';
...
CREATE INDEX api_key_usage_created_at_idx ON api_key_usage(created_at DESC);
